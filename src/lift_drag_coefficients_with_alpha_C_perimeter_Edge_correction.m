...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 绘制气动力系数
% 2015-JFM-Hao_Liu-A CFD-informed quasi-steady model of flapping-wing aerodynamics_hiP
C_L2a=sin(2*alpha_4*pi/180);
C_L1=-6.5;
C_L2=0.5;
C_L_nakata=C_L1*((alpha_4*pi/180).^3-pi/2*(alpha_4*pi/180).^2)+C_L2*((alpha_4*pi/180).^2-pi/2*(alpha_4*pi/180));
C_L_nakatamax=max(abs(C_L_nakata));
% C_D_min=;
% C_D_max=;
% C_D_nakata=C_D_min*(cos(alpha_4*pi/180)).^2+C_D_max*(sin(alpha_4*pi/180)).^2;
%%%%%%%%%%%%%%%%%%%%%%%%%
figure(15)
hold on
C_f=plot(alpha_4,C_L2a,'k-',alpha_4,abs(C_L_nakata/C_L_nakatamax),'g-.');
% C_f=plot(alpha_4,C_L2a,'k-');
xlabel('\it\alpha (deg.)')
ylabel('\it C_L (\alpha )')
title('Aerodynamic coefficients of lift \itvs. \alpha \rm for flapping wing')
legend('\itC_{L,nakata}')
set(C_f,'LineWidth',2)
grid on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
